...
## Scoring output (`scoring.score.score()`) ΓÇö owned by scoring/
...
respective track and aren't documented here yet ΓÇö confirm with whoever owns
...
- Coordinates are top-left origin, pages are zero-indexed ΓÇö unchanged from
  what's passed in.
  ...
  "candidates": [
    {
      ...
      "checksum_valid": True,       # True = passed; None = type has no
                                    # checksum. Never False ΓÇö failures are
                                    # dropped, not emitted.
      "match_source": "regex",
...
- Overlapping spans are resolved before emission ΓÇö longest span wins, then
